...
> "Hibernate는 Session 인스턴스를 SessionFactory로 만든다. SQLAlchemy는 unit-of-work 개념에 더 집중하는데, 처음엔 이해·사용이 어렵지만 나중에 우연한 commit-타이밍 버그를 거의 0으로 줄여주는 가치를 깨닫게 된다."

그렇다면 어디서부터 손을 대야 할까. 이 장에선 도서 카탈로그를 예제로 잡고, 모델 정의 → 쿼리 → 세션 다루기 → 레포지토리 직접 짜기 → Alembic 마이그레이션 → 폴더 구조 약속까지 한 호흡으로 짚어보자. JPA 사고가 그대로 가는 부분, 살짝 비틀어야 하는 부분, 완전히 다시 짜야 하는 부분을 하나씩 분리해 보자.
...
도서 카탈로그를 짜보자. 도메인은 셋이다. `Book`, `Author`, `Category`. 다대다(Book ↔ Category)와 다대일(Book → Author) 관계가 한 번씩 등장한다. JPA로 짰다면 익숙한 어노테이션 더미가 등장하는 자리다.
...
> "의존성의 finally 절에서 commit을 할 수 없다 — 그 코드는 경로 함수가 끝나고 결과가 클라이언트에 반환된 뒤에 실행된다."
...
손에 익혀 두자. **`-to-many는 `selectinload`, `-to-one은 `joinedload`*. 헷갈리면 `selectinload`를 기본으로.
...
### Unit of Work — Matthew Brown의 비판을 한 번 짚어두자

여기서 한 가지 더 짚어두자. FastAPI 진영엔 "DB 세션을 의존성으로 라우트 끝까지 끌고 다니는 게 안티패턴"이라는 비판이 있다. 라우트가 길어지면 *요청 내 longtail 트랜잭션*이 생기고, 어디서 commit이 일어났는지 추적이 힘들어진다는 주장이다.
...
다만 본 책의 5장 예제는 *서비스 레이어에서 명시 트랜잭션* 패턴으로 가르친다. UoW는 6장에서 트랜잭션 챕터의 본격 주제로 다시 다룬다. 일단은 "이런 대안이 있다"는 사실만 머리에 넣어두자.
...
본 장 마지막으로 한 그림을 박아두자. 이 책의 **모든 작은 예제**가 따라가는 폴더 구조의 표준이다. 4·6·7·9·10장 어디서 새 도메인이 등장하더라도, 우리는 이 모양을 반복한다. 12장의 통합 프로젝트는 이걸 *이미 안다고 가정*한다.
...
5장은 이 책에서 가장 *손에 익혀야 할 패턴이 많은* 장이다. 정리해 두자.
...
- **Lazy 함정**: `DetachedInstanceError`는 `selectinload`/`joinedload`로 사전 차단하자. `-to-many는 selectinload, -to-one은 joinedload`.
