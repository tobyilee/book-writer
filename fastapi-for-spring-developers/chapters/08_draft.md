...
2024년에 PEP 703이 받아들여졌다. 한 줄로 요약하면 *GIL을 제거한 빌드를 옵션으로 제공*한다. Python 3.13(2024년 10월)부터 *experimental* free-threaded 빌드(`python3.13t`)가 함께 배포된다. 2026년 현재, free-threaded 빌드는 *베타*에 가까운 단계다 — 표준 라이브러리는 거의 호환되지만, *C 확장 모듈*(NumPy, Pillow, psycopg, lxml 등)은 호환이 진행 중이다. 프로덕션 권장은 아직 아니다.
