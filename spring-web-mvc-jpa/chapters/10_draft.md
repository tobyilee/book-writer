...
JWT의 진짜 매력은 구조가 아니라 *운영 방식*에 있다. 9장에서 잠깐 비춘 9장 끝의 예고처럼, JWT는 **stateless(상태 없음)** 인증의 대표 선수다. 무슨 뜻일까?
...
여기서 잠깐, 백엔드 코드로 들어가기 전에 프런트 개발자인 당신이 *직접* 결정해야 할 문제를 하나 짚고 가자. 서버가 발급한 그 토큰, 브라우저의 *어디에* 저장할 것인가?

흔한 후보가 둘이다. `localStorage`냐, `httpOnly` 쿠키냐.

`localStorage`는 편하다. 자바스크립트로 `localStorage.setItem('token', ...)` 한 줄이면 저장되고, 요청 보낼 때 꺼내서 `Authorization` 헤더에 실으면 된다. 많은 React 튜토리얼이 이 방식을 보여준다. 그런데 정말 이걸로 충분할까? 한 가지 찜찜한 구석이 있다. `localStorage`는 그 페이지의 *모든 자바스크립트*가 읽을 수 있다. 만에 하나 XSS(스크립트 주입) 공격으로 악성 스크립트가 페이지에 끼어들면, 토큰을 그대로 훔쳐갈 수 있다. 출입증을 책상 위에 펼쳐둔 셈이다.

반면 `httpOnly` 쿠키는 자바스크립트가 *읽지 못하도록* 봉인된 쿠키다. 브라우저가 요청에 자동으로 실어 보내주지만, 코드로는 꺼낼 수 없다. XSS가 나도 토큰을 직접 훔치긴 어렵다. 대신 쿠키는 또 다른 공격(CSRF)에 대한 대비가 필요하고, 5장에서 우리가 씨름한 CORS의 `allowCredentials` 설정과 얽힌다.

지금 이 자리에서 "무조건 이게 정답"이라고 못 박진 않겠다. 이건 JWT냐 세션이냐의 선택과도 맞물려 있어서, 11장에서 트레이드오프를 정면으로 펼쳐놓고 함께 판단할 것이다. 다만 오늘 기억해둘 건 이것이다 — **토큰을 어디 두느냐는 프런트의 사소한 구현 디테일이 아니라 보안 결정**이라는 것. (프런트 ↔ 백엔드 대응이 헷갈리면 부록 A 대조표의 '토큰 저장' 항목을 함께 보자.) 이 장의 코드 예시는 설명을 단순하게 하려고 `Authorization: Bearer` 헤더 방식을 쓰지만, 그 선택의 무게는 잊지 말자.
...
> **(사실 확인 필요)** 위 `jjwt` 0.12.x 계열 API(`Jwts.builder().subject()/signWith()/compact()`, `Jwts.parser().verifyWith().build().parseSignedClaims()`, `Keys.hmacShaKeyFor`)의 메서드 시그니처가 저술 시점 최신 버전 기준으로 정확한지 레퍼런스 §3·§5 및 jjwt 공식 릴리스 노트와 대조 바람. jjwt는 0.11→0.12에서 빌더/파서 API가 크게 바뀌어, 인터넷·AI 예제가 구버전(`Jwts.parserBuilder()`, `setSubject()` 등)을 줄 위험이 큰 영역이다.
...
> **(사실 확인 필요)** `OncePerRequestFilter`, `SecurityContextHolder`, `UsernamePasswordAuthenticationToken`의 패키지·시그니처가 Spring Security 6.x(`jakarta.servlet.*` 기반) 기준으로 정확한지 레퍼런스 §3·§5와 대조 바람. 특히 `HttpServletRequest`가 `javax.servlet`이 아니라 `jakarta.servlet` 네임스페이스인지 확인 필요 — 이것이 바로 이 책이 거듭 경고해온 javax→jakarta 함정이다.

복잡해 보이지만 하는 일은 9장에서 그린 그대로다. `Authorization` 헤더에서 `Bearer ` 뒤의 토큰을 떼어내고, 아까 만든 `JwtTokenProvider`로 검증하고, 통과하면 "이 요청은 이 사용자가 보냈다"를 `SecurityContextHolder`에 등록한다. 이렇게 등록해두면, 그다음 인가 필터가 "이 사용자가 이 경로에 들어갈 권한이 있나"를 따질 때 이 정보를 본다. 마지막 `chain.doFilter`로 다음 검문소에게 요청을 넘긴다 — 줄을 따라 흘러가는 것이다.
...
> **(사실 확인 필요)** 위 6.x 설정의 `csrf(...)`·`sessionManagement(...)`·`SessionCreationPolicy.STATELESS`·`authorizeHttpRequests`·`requestMatchers`·`addFilterBefore` 람다 DSL 시그니처가 Spring Security 6.x 기준으로 정확한지 레퍼런스 §3·§5(자료 8: SecurityFilterChain)와 대조 바람.
...
기억해두자. 보안 설정만큼은 AI에게 받은 코드를 *그대로 믿어선 안 된다*. 받자마자 세 가지를 확인하는 습관을 들이는 편이 낫다. 첫째, `WebSecurityConfigurerAdapter`를 상속하고 있지 않은가. 둘째, `authorizeRequests`·`antMatchers` 같은 옛 이름이 보이지 않는가. 셋째, import가 `javax.*`가 아니라 `jakarta.*`인가. 이 세 가지만 걸러내도 AI 코드의 가장 흔한 함정은 피한다. 더 촘촘한 점검 목록은 부록 B 신선도 체크리스트에 정리해두었으니 곁에 두고 쓰자.
...
> **AI 페어코딩 학습 포인트**
>
> 이 장이 신선도 함정의 클라이맥스인 만큼, AI 활용법도 한 단계 단단해질 차례다. JWT·Security 설정을 AI에게 시킬 때는 프롬프트에 "Spring Boot 3.x, Spring Security 6.x, jakarta 네임스페이스, jjwt 0.12 기준" 같은 버전 좌표를 *먼저* 못 박자. 그래도 AI는 구버전을 섞어 줄 때가 많다. 받은 코드는 이 장에서 익힌 before/after의 눈으로 거른다 — `WebSecurityConfigurerAdapter` 상속 여부, `authorizeRequests`/`antMatchers` 같은 옛 이름, jjwt의 `parserBuilder()`·`setSubject()` 같은 구버전 빌더, 그리고 `javax.*` import. 하나라도 보이면 그건 십중팔구 시점이 어긋난 코드다. 의심스러우면 AI에게 "이 코드 Spring Security 6.x에서 컴파일돼? `WebSecurityConfigurerAdapter`는 제거된 걸로 아는데"라고 *되물어* 스스로 교정하게 하는 것도 좋은 방법이다. 점검 목록은 부록 B 신선도 체크리스트로 확인하자.
