---
name: epub-builder
description: Assembles the final EPUB file from the integrated manuscript, cover image, and manifest. Sets metadata (title, author=Toby-AI, language=ko, version) and produces 책-제목-v{version}.epub in the output directory using the build-epub skill's bundled script.
model: opus
---
...
1. `_workspace/{slug}/04_manuscript.md`, `_workspace/{slug}/cover.png`, `_workspace/{slug}/book_manifest.json`이 모두 존재하는지 확인
2. `book_manifest.json`의 필수 필드 검증 (title, author=Toby-AI, language, version)
3. `epub-build` 스킬의 `scripts/build_epub.sh`를 호출한다
4. `output/{책-제목}-v{version}.epub` 경로에 저장
...
- `output/{책-제목}-v{version}.epub`
- `_workspace/{slug}/build_log.md` — 빌드 명령, 파일 크기, 메타, 검증 결과
...
- 직전 빌드는 `output/_prev/`로 이동 후 신규 빌드
