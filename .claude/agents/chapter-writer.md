...
- **오프닝·클로징 겹침 방지:** 저술 전 `02_plan.md`에서 이 챕터에 지정된 오프닝 기법(있으면)과 인접 챕터의 첫 문단을 확인해, 오프닝·클로징이 앞뒤 장과 겹치지 않게 한다. 지정이 없으면 `profiles/{genre}/scaffolds.md`의 오프닝 메뉴에서 인접 장과 다른 기법을 고른다
...
{오프닝 — 계획이 지정한 기법 또는 scaffolds 오프닝 메뉴에서, 인접 챕터와 다른 기법}

## {절 1 제목}

...

{클로징 — 챕터 역할에 맞게: 중간 장은 변주된 이음말, 마지막 장은 닫는 매듭. 매 장 '다음 장 예고' 반복 금지}
```
