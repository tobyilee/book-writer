...
주제·주요 내용·대상 독자를 받아 Toby 스타일의 완성된 EPUB을 산출하는 전 과정을 조율한다. 각 Phase에서 전문 에이전트를 호출하고, 중간 산출물을 `_workspace/` 하위에 축적한 뒤 최종적으로 `output/` 경로에 EPUB을 만든다.
...
3. `_workspace/{slug}/`의 존재 여부를 확인한다.
   - **미존재** → 초기 실행, Phase 1부터 순차 실행
   - **존재 + 사용자가 부분 수정 요청** (예: "챕터 3만 다시", "계획만 수정") → 부분 재실행, 해당 Phase만 재호출
   - **존재 + 새 입력 제공** → 기존 `_workspace/{slug}/`를 `_workspace/{slug}_prev-{timestamp}/`로 이동 후 새 실행
...
**입력:** 주제, 주요 내용, 대상 독자
**출력:** `_workspace/{slug}/01_reference.md` — 리서치 종합 문서 (섹션: 개념·정의, 주요 관점, 사례, 논쟁점, 참고문헌)
...
**입력:** 주제, 주요 내용, 대상 독자, `_workspace/{slug}/01_reference.md`
**출력:** `_workspace/{slug}/02_plan.md` — 책 구조 설계 문서
- 책 제목 후보 3개
...
**산출물:** `_workspace/{slug}/02_plan.md` (갱신됨) + `_workspace/{slug}/03_review_log.md` (리뷰 기록)
...
3. 각 `chapter-writer`는 자기 챕터 초안을 쓰고 `_workspace/{slug}/chapters/{NN}_draft.md`에 저장한 뒤 `SendMessage`로 `style-guardian`에게 리뷰를 요청한다.
...
6. 모든 챕터 완료 후 `editor`가 전환부를 점검하고 `_workspace/{slug}/04_manuscript.md`에 통합 원고를 만든다.
7. 팀을 해체한다.
...
- `cover-designer` → 표지 이미지 생성, `_workspace/{slug}/cover.png` 저장
- `epub-builder`는 cover가 준비된 후에 호출 (순서 의존) → `output/{책-제목}-v{version}.epub` 생성
...
| 전략 | 용도 |
|------|------|
| 파일 기반 (`_workspace/`) | 모든 Phase 간 산출물 전달, 감사 추적 |
