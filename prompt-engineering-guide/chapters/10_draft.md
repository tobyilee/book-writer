...
안타깝게도 그렇지 않다. 사람은 — 그리고 나중에 보겠지만 기계도 — 먼저 본 쪽이나 왼쪽에 놓인 쪽을 은근히 더 후하게 평가하는 경향이 있다. 이걸 **위치 편향(position bias)**이라고 부른다. A를 늘 왼쪽에, B를 늘 오른쪽에 두고 채점하면, 그 자리 배치 자체가 점수에 슬그머니 끼어든다. LLM-as-judge 연구로 유명한 Zheng 등의 2023년 논문도 이 위치 편향을 핵심 위험으로 짚었다 [Zheng 2023, arXiv:2306.05685].
