...
OKKY의 그 사례가 여기서 나온다. 같은 컬럼으로 복합 인덱스를 만들었는데, 컬럼 순서를 바꿨더니 11초가 0.1초가 됐다.
...
그런데 그 후 흥미로운 일이 있었다. 인덱스를 추가했더니 이번엔 옵티마이저가 JOIN 컬럼 인덱스를 "너무 우선"해서 다른 인덱스를 무시하는 현상이 생겼다. 이것이 인덱스 추가로 끝이 아닌 이유다. 인덱스를 추가한 뒤 `EXPLAIN`으로 옵티마이저가 어떤 인덱스를 선택했는지, 예상대로 움직이는지를 확인해야 한다. 5장이 이 확인 절차를 다룬다.
