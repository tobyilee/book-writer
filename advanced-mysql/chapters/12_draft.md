...
DB를 운영해본 사람은 안다. 숫자가 녹색이어도 안심할 수 없다. DB는 말이 없다. 이상 신호를 보내도 우리가 제대로 듣지 못하면 그냥 지나친다. 이 장에서는 DB가 보내는 신호를 어떻게 듣고, 장애가 일어났을 때 어떻게 복구하며, 그 복구를 일상으로 만드는 방법을 함께 살펴보자.
...
실용적인 원칙은 이렇다. **필요한 것만, 필요한 순간에 켜라.**
...
`Threads_running`이 CPU 코어 수를 크게 넘으면 대기가 쌓이고 있다는 뜻이다. 이때 `sys.processlist`나 `SHOW PROCESSLIST`로 어떤 쿼리가 오래 실행 중인지 확인하자.
...
슬로우 쿼리 로그가 쌓이면 `pt-query-digest`로 분석한다. 이 도구는 같은 패턴의 쿼리를 그룹핑해 실행 횟수, 평균 시간, 최대 시간, 실행 시간 합계를 보여준다.
...
`--single-transaction`은 InnoDB에서 일관된 스냅샷을 덤프하기 위한 핵심 옵션이다. REPEATABLE READ 격리수준에서 읽기 전용 트랜잭션을 열어 덤프하기 때문에 다른 쓰기 트랜잭션을 차단하지 않는다. MyISAM 테이블이 없는 한 이 옵션을 항상 켜는 편이 낫다.
...
실수로 `DROP TABLE`이나 `DELETE FROM` 을 날렸다면:
...
DR 설계 시 고려해야 할 RTO/RPO 약속의 두 축이 있다.
...
12장에서 메이저 업그레이드 절차를 다루지는 않는다(14장이 그 몫이다). 하지만 업그레이드를 언제 결심해야 하는지의 신호는 운영 일상에서 만난다.
...
**인증 플러그인 경고.** `mysql_native_password` 폐기 예고가 뜨거나, 클라이언트 드라이버가 지원하지 않는 인증 방식을 요구하면 업그레이드 타이밍을 검토해야 한다.
...
8.0부터 기본 인증 플러그인은 `caching_sha2_password`다. MySQL Connector/J 8.0.17 이상, Python mysql-connector 8.0.17 이상은 이미 지원한다. 하지만 레거시 드라이버를 쓰는 시스템이라면 업그레이드 전에 반드시 확인해야 한다.
...
MySQL Enterprise Audit과 Percona Audit Plugin이 대표적이다. 어느 쪽이든 로그 파일 크기가 빠르게 커질 수 있으니 보존 정책을 함께 정의해야 한다.
...
버퍼풀 히트율과 `Threads_running`을 매일 챙기자. 슬로우 쿼리 로그를 켜두고 주기적으로 `pt-query-digest`로 분석하자. 복제 lag은 `Seconds_Behind_Source`보다 PS의 timestamp 비교가 더 정확하다.
