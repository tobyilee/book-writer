...
*챕터 작성 메모: 02_plan §3 6장 사양 충실 반영. L4/L7 / HAProxy·NGINX·Envoy 비교 / API Gateway 패턴 / Service Mesh (Istio·Linkerd·Ambient) / mTLS 정량 비용 (+166%/+33%/+8%) / 5명 팀 ≠ K8s+Istio 논쟁 G / mesh 도입 4질문 분해 도구 / 한국 사례 / 새벽 3시 7체크리스트 sidebar. W5·W44·논쟁 G(K8s) 인용. 한국 사례 일부와 Netflix 트래픽 비율에 "(검증 필요)" 라벨 (1장 baseline 패턴). 분량 약 18p 추정 (한국어 ~5000자). style-guardian 라운드 1 리뷰 처리: Critical 1건은 team-lead 챕터 보드 재정렬로 무효 확정 (본문 "9장 보안"·"14장 on-call"·"19장 결제" callback 모두 원래 정답). Should 3건 + Nice 3건 모두 반영. Should: Netflix 95% → (검증 필요), cluster 메모리 100~200GB 비례감 한 줄, 한국 사례 절 도입 (검증 필요) 한 줄 제거. Nice: wrapper layer 풀이 / framework→분해 도구 / frame→틀 / +166% 강조 굵게 / 회수 anchor "X가 아니라 Y" 패턴 1장 L201 baseline 일치.*
