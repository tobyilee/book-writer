...
*챕터 작성 메모: 02_plan §3 부록 A 사양 충실 반영. tribal knowledge 18선 카드 형식 (증상/원인/즉시 mitigation/근본 fix 4줄씩) + 한국 환경 야사 3개 (0시 트래픽 농담, SK C&C 화재 회고, 우아한형제들 새벽 운영 패턴) + on-call 휴머니즘 (Charity Majors 인용 + alert 절반 자동화 + runbook 의무화) + 이력서용 기술 도입 함정 사례 2개 (mesh / NoSQL) + 배포·마이그레이션 실패담 2개 (한국 핀테크 blue-green / Slack 2021 outage) + AI/LLM 한 페이지 "다음 책의 약속" (6개 영역 매핑) + 책의 한계 + 마지막 한 줄. community.md tribal 18선 원자료, 한국 1·6·9·10, 휴리스틱 9, 패턴 8 인용. 한국 사례 일부와 사고 사례 디테일에 "(검증 필요)" 라벨 (1장 baseline 패턴). 분량 약 15p 추정 (한국어 ~7300자). 부품·패턴 callback: 1장(connection pool / VACUUM), 2장(NoSQL tombstone), 3장(캐시 cluster), 4장(Kafka rebalance), 5장(ES refresh), 6장(service mesh ALB 대안), 8장(NTP 시간), 9장(망분리 break-glass), 10장(timeout / circuit breaker / idempotency), 11장(Event Sourcing), 12장(multi-region), 14장(progressive rollout / blameless / alert / chaos), 15장(Lambda/Kappa LLM 변주). 0장 도입부 "동료의 어깨" 호응으로 책 전체를 닫는다.*
