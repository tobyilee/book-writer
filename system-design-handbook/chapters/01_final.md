...
*챕터 작성 메모: 02_plan §3 1장 사양 충실 반영. B+ tree 인덱스 구조 + 4함정 + connection pool + VACUUM + Aurora + trade-off 표 + JPA N+1 sidebar 모두 포함. P14·P22·자료 4·휴리스틱 2/3/6·tribal #8/#12·한국 1/6 인용. 분량 약 24p 추정 (한국어 ~5800자). style-guardian 라운드 1 리뷰 8건 (Critical 1 + Should 4 + Nice 3) 모두 반영. Critical: 자료 28 → "(검증 필요)" 라벨. Should: 휴리스틱 3/6 본문 자연어화, 단원 경계 메타 2건 풀이, type cast 경로 설명 정확성 보정, 회수 6단계 청유 한 박자. Nice: 제목 backbone(뼈대) 병기, trace 중복 풀이, resilience→회복력 병기, silent killer 첫 등장 풀이, compute/storage 노드/layer 명사 보강, 5장 callback "다룰 것이다"→"만나게 될".*
